...
static void producer_resposta(char* result) {
	...
	log_message(LOG_FILE, "fez declara̤oes de variaveis");
	...
	if (rd_kafka_produce(
		/* Topic object */
		rkt2,
		/* use o particionador embutido para selecionar a parti̤̣o*/
		RD_KAFKA_PARTITION_UA,
		0,
		/* Carga ̼til da mensagem (valor) e comprimento */
		result, size,
	...
	if (size > tamanho_da_mensagem) {
		...
		for (i = 0; i < numero_d_partes; i++) {
			...
			if (i == numero_d_partes - 1) {
				...
				fseek(farquivo, ponteiro_d_bite, SEEK_SET);/*foi pra posi̤̣o de 100mb*/
...
DWORD WINAPI ServiceWorkerThread(LPVOID lpParam) {
	...
	while (WaitForSingleObject(g_ServiceStopEvent, 0) != WAIT_OBJECT_0) {
		...
		if (rd_kafka_conf_set(conf, "bootstrap.servers", brokers, errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
			log_message(LOG_FILE, "Erro configura̤̣o bootstrap.servers");
		...
		if (rd_kafka_conf_set(conf, "group.id", groupid, errstr, sizeof(errstr)) != RD_KAFKA_CONF_OK) {
			log_message(LOG_FILE, "Erro configura̤̣o group.id");
			log_message(LOG_FILE, errstr);
			rd_kafka_conf_destroy(conf);
			continue;
		}
