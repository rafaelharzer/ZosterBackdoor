...
DWORD WINAPI producer_fun(void* argument) {
			...
			*nome_arquivo++;				
			...
			if(size > tamanho_da_mensagem){
					...
					numero_d_partes = (size/tamanho_da_mensagem)+1; /*quantas partes o arquivo vai ter*/
            			...
            			int i;
            			arquivos_mandar = (char*) malloc(tamanho_da_mensagem);
...
DWORD WINAPI consumer_fun(void *broker) {
	...
	while(1){
		...
		if (strstr(keynamefile, ".P.0") != NULL) {
			...
			char* mensagem = (char*) rkmessage->payload;
